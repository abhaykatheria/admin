...
export default function PaymentCollection() {
    ...
    useEffect(() => {
        ...
        window.scrollTo(0, 0);
        db.collection("payment_collection").onSnapshot((snapshot) => {
            const data = [];
            snapshot.forEach((doc) => data.push({ ...doc.data(), id: doc.id,studentId: doc.data().id }));
            console.log(data); // <------
            data.sort((a, b) =>
...
async function deleteDoc(assignment) {
    ...
    await app.firestore().collection('payment_collection').doc(assignment.id).delete()
}

async function updateDues(dues, assignment) {
    console.log(dues)

    await app.firestore().collection('students').doc(assignment.studentId).update({
